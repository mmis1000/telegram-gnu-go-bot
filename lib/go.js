...
function createConfigKeyBoard(currentRow, currentMode, currentLevel) {
  ...
  return [rowOptions, modeOption, levelOptions];
}
...
[Source codes](https://github.com/mmis1000/telegram-gnu-go-bot)
  ...
  if (text.match(/^\/(setting)(@|$|\s)/)) {
    api.sendMessage(message.chat.id, 'setting', null, {
      reply_markup: JSON.stringify({inline_keyboard: createConfigKeyBoard(sess.params.boardsize, sess.mode, sess.params.level)})
    })
  }
})
  ...
  switch (args[1]) {
    ...
    case "claim_black":
      if (session.game && session.users[query.from.id].faction) {
        ...
        return api.answerCallbackQuery(query.id, null, {
          text: 'You canclaim faction in 2p mode only'
        })
      } else {
        session.users[query.from.id].faction = 'black';
        return api.answerCallbackQuery(query.id, null, {
          text: 'Your factor has been changed to black'
        })
      }
      break;
    case "claim_white":
      if (session.game && session.users[query.from.id].faction) {
        ...
        return api.answerCallbackQuery(query.id, null, {
          text: 'Your factor has been changed to white'
        })
      }
      break;
  }
...
inlineKeyboard.on('setting', function (args, query) {
  ...
  api.editMessageReplyMarkup(chat_id, msg_id, JSON.stringify({
    inline_keyboard: createConfigKeyBoard(session.params.boardsize, session.mode, session.params.level)
  }))
})
